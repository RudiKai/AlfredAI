...
//|                 v1.1 - Fixed Mitigation Check Error              |
...
input int    RefreshRateSeconds = 5;       // How often to redraw zones (in seconds)
input double MinImpulseMovePips = 10.0;    // Minimum size of an impulse move to form a zone

// --- Zone Detection Logic ---
input group "--- Zone Detection Logic ---"
input int  ZoneLookback         = 50;     // How many bars back to search for zones
...
void OnDeinit(const int reason)
{
   ...
   string del_prefixes[] = { "DZone_", "SZone_", "MagnetLine_" };
   string tf_suffixes[] = { "LTF", "M15", "M30", "H1", "H2", "H4", "D1" };
   
   for(int i=0; i<ArraySize(del_prefixes); i++)
   {
      for(int j=0; j<ArraySize(tf_suffixes); j++)
      {
         ObjectDelete(0, del_prefixes[i] + tf_suffixes[j]);
      }
   }
   ChartRedraw();
}
...
//+------------------------------------------------------------------+
//| OnCalculate: Only checks for mitigated zones on new bars.        |
//+------------------------------------------------------------------+
int OnCalculate(const int rates_total,
                ...
                const int      &sp[])
{
   // --- Only run this logic once per bar ---
   static datetime lastBarTime = 0;
   if(t[rates_total-1] != lastBarTime)
   {
      if(EnableBreakoutRemoval)
      {
         // FIX: Pass the close prices array 'c' to the function
...
//+------------------------------------------------------------------+
//| Chart Event: Redraws on manual chart changes for responsiveness. |
//+------------------------------------------------------------------+
...
void DrawMagnetLine()
{
    // Simplified logic for example; a real version might weigh TFs differently
...
// FIX: Added 'const double &close[]' to the function signature
void CheckForMitigations(int rates_total, const double &low[], const double &high[], const double &close[])
{
   if(rates_total < 2) return;
   
   // FIX: Correctly access the close price from the passed-in array 'close'
