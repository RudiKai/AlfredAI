...
// MODIFIED: Confidence Matrix logic now uses live data and a new scoring formula.
...
// MODIFIED: MatrixScoreToColor is no longer needed and has been removed.

// NEW: Helper function to return a color based on the numerical confidence score.
color GetConfidenceColor(int score)
{
   if(score >= 16) return (color)ColorToARGB(clrDodgerBlue, 120); // Blue
   if(score >= 10) return (color)ColorToARGB(clrLimeGreen, 120);  // Green
   if(score >= 5)  return (color)ColorToARGB(clrGoldenrod, 100);  // Yellow
   return (color)ColorToARGB(clrOrangeRed, 120);                   // Red
}
...
CreateLabel("matrix_hdr_zone", "Zone", x_col1 + 100, y_offset, COLOR_HEADER);
      CreateLabel("matrix_hdr_score", "Score", x_col1 + 160, y_offset, COLOR_HEADER); // Changed "Magnet" to "Score"
      y_offset += SPACING_MEDIUM;
for(int i = 0; i < ArraySize(g_matrix_tfs); i++)
      {
...
CreateLabel("matrix_zone_" + tf, "---", x_col1 + 100, y_offset, COLOR_NA);
         CreateLabel("matrix_magnet_" + tf, "---", x_col1 + 160, y_offset, COLOR_NA); // This will display the score
y_offset += SPACING_MEDIUM;
      }
...
string tf_str = g_matrix_tf_strings[i];
         ...
         string font_style = (data.score >= 10) ? "Arial Bold" : "Arial"; // Using 10 as threshold for "strong"
