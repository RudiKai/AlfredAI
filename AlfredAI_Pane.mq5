//+------------------------------------------------------------------+
//|                        AlfredAI_Pane.mq5                         |
//|             v1.8.5 - "Alfred Says" Module Added                  |
//|                                                                  |
...
#property version "1.8.5" // UPGRADED: Version reflects "Alfred Says" module
...
input bool ShowAlfredSays = true; 
// NEW: Toggle for the "Alfred Says" module
...
void UpdateLiveDataCaches()
{
   ...
   for(int i = 0; i < ArraySize(g_timeframes); i++)
   {
      ...
      if(supdem_handle != INVALID_HANDLE)
      {
         ...
         if(CopyBuffer(supdem_handle, 1, 0, 1, magnet_buffer) > 0) g_supdem_cache[i].magnet_level = magnet_buffer[0];
         if(CopyBuffer(supdem_handle, 2, 0, 1, strength_buffer) > 0) g_supdem_cache[i].strength = strength_buffer[0];
         if(CopyBuffer(supdem_handle, 3, 0, 1, fresh_buffer) > 0) g_supdem_cache[i].freshness = fresh_buffer[0];
         if(CopyBuffer(supdem_handle, 4, 0, 1, volume_buffer) > 0) g_supdem_cache[i].volume = volume_buffer[0];
         if(CopyBuffer(supdem_handle, 5, 0, 1, liq_buffer) > 0) g_supdem_cache[i].liquidity = liq_buffer[0];
      }
...
{
   int index = GetCacheIndex(tf);
   if(index != -1) return g_supdem_cache[index].zone;
   return ZONE_NONE;
}
...
{
   int index = GetCacheIndex(tf);
   if(index != -1) return g_supdem_cache[index].magnet_level;
   return 0.0;
}
...
{
   int index = GetCacheIndex(tf);
   if(index != -1) return g_hud_cache[index].zone_active;
   return false;
}
         ...
         if(row.score >= STRONG_CONFIDENCE_THRESHOLD && row.bias == BIAS_BULL)
         {
            ...
            if(zone_data.zone == ZONE_DEMAND)
            {
               ...
               if(current_score > best_score)
               {
                  ...
                  if(zone_data.liquidity > 0.5) best_reason += " (Liq. Grab)";
               }
            }
         }
      }
...
{
   RiskModuleData data;
   data.risk_percent = 1.0; data.position_size = 0.10;
   int rand_val = MathRand() % 3;
   switch(rand_val) { case 0: data.rr_ratio = "1 : 1.5"; break; case 1: data.rr_ratio = "1 : 2.0"; break; default: data.rr_ratio = "1 : 3.0"; break; }
   return data;
}
SessionData GetSessionData()
{
   SessionData data;
   MqlDateTime dt; TimeCurrent(dt); int hour = dt.hour;
   ...
   int rand_val = MathRand() % 3;
   switch(rand_val) { case 0: data.volatility = VOLATILITY_LOW; break; case 1: data.volatility = VOLATILITY_MEDIUM; break; default: data.volatility = VOLATILITY_HIGH; break; }
   return data;
}
int GetUpcomingNews(NewsEventData &news_array[])
{
   static NewsEventData all_news[] = { {"14:30", "USD", "Non-Farm Payrolls", IMPACT_HIGH}, {"16:00", "EUR", "CPI YoY", IMPACT_MEDIUM}, {"22:00", "NZD", "Official Cash Rate", IMPACT_HIGH}, {"01:30", "AUD", "Retail Sales MoM", IMPACT_LOW} };
...
AlertData GetAlertCenterStatus()
{
   ...
   int strong_count = 0, medium_count = 0;
   for(int i = 0; i < ArraySize(g_matrix_tfs); i++)
   {
      MatrixRowData row = GetConfidenceMatrixRow(g_matrix_tfs[i]);
   ...
   if(strong_count > 0) { alert.status = ALERT_STRONG; alert.text = "✅ STRONG ALIGNMENT — High-Conviction Setup"; }
   else if(medium_count > 0) { alert.status = ALERT_PARTIAL; alert.text = "⚠️ Partial Alignment — Watch for Entry Trigger"; }
   else { alert.status = ALERT_NONE; alert.text = "⏳ No Signal — Standby"; }
   return alert;
}
...
void CreatePanel()
{
   ...
   if(ShowNewsModule)
   {
      ...
      for(int i = 0; i < MAX_NEWS_ITEMS; i++) { string idx = IntegerToString(i); CreateLabel("news_time_" + idx, "", x_col1, y_offset, COLOR_TEXT_DIM); CreateLabel("news_curr_" + idx, "", x_col1 + 40, y_offset, COLOR_NEUTRAL_TEXT); CreateLabel("news_event_" + idx, "", x_col1 + 75, y_offset, COLOR_NEUTRAL_TEXT); CreateLabel("news_impact_" + idx, "", x_col1 + 180, y_offset, COLOR_NEUTRAL_TEXT, FONT_SIZE_NORMAL, ANCHOR_RIGHT); y_offset += SPACING_MEDIUM; }
   ...
   if(ShowEmotionalState)
   {
      ...
      y_offset += SPACING_MEDIUM; DrawSeparator("sep_emotion", y_offset, x_offset);
   }
   ...
   if(ShowAlertCenter)
   {
      ...
      y_offset += SPACING_MEDIUM; DrawSeparator("sep_alert", y_offset, x_offset);
   }
   ...
   if(ShowPaneSettings)
   {
      ...
      CreateLabel("setting_matrix_prefix", "📊 Bias Matrix:", settings_x1, y_offset, COLOR_HEADER); CreateLabel("setting_matrix_value", "---", settings_x2, y_offset, COLOR_NA); y_offset += SPACING_MEDIUM;
      CreateLabel("setting_magnet_prefix", "📉 Magnet Summary:", settings_x1, y_offset, COLOR_HEADER); CreateLabel("setting_magnet_value", "---", settings_x2, y_offset, COLOR_NA); y_offset += SPACING_MEDIUM;
   ...
   // --- TF Biases Section ---
   CreateLabel("biases_header", "TF Biases & Zones", x_col1, y_offset, COLOR_HEADER, FONT_SIZE_HEADER); CreateLabel("biases_toggle", g_biases_expanded ? "[-]" : "[+]", x_toggle, y_offset, COLOR_TOGGLE, FONT_SIZE_HEADER); y_offset += SPACING_MEDIUM;
   if(g_biases_expanded)
   {
      ...
      for(int i = 0; i < ArraySize(tf_bias_strings); i++) { string tf = tf_bias_strings[i]; CreateLabel("biases_" + tf + "_prefix", tf + ":", x_col_tf, y_offset, COLOR_HEADER); CreateLabel("biases_" + tf + "_value", "---", x_col_bias, y_offset, COLOR_NA); CreateLabel("zone_" + tf + "_value", "---", x_col_zone, y_offset, COLOR_NA); CreateLabel("magnet_" + tf + "_value", "---", x_col_magnet, y_offset, COLOR_NA); y_offset += SPACING_MEDIUM; }
   }
   y_offset += SPACING_SEPARATOR - (g_biases_expanded ? SPACING_MEDIUM : 0); DrawSeparator("sep1", y_offset, x_offset);
   ...
   CreateLabel("zone_qual_str_prefix", "Strength:", x_qual_1, y_offset, COLOR_HEADER); CreateLabel("zone_qual_str_value", "N/A", x_qual_2, y_offset, COLOR_NA); y_offset += SPACING_MEDIUM;
   CreateLabel("zone_qual_fresh_prefix", "Fresh:", x_qual_1, y_offset, COLOR_HEADER); CreateLabel("zone_qual_fresh_value", "N/A", x_qual_2, y_offset, COLOR_NA); y_offset += SPACING_MEDIUM;
   ...
   if(ShowZoneHeatmap)
   {
      ...
      for(int i = 0; i < ArraySize(g_heatmap_tf_strings); i++) { string tf = g_heatmap_tf_strings[i]; CreateLabel("heatmap_tf_" + tf, tf, heatmap_x, y_offset, COLOR_HEADER, FONT_SIZE_NORMAL, ANCHOR_CENTER); CreateLabel("heatmap_status_" + tf, "-", heatmap_x, y_offset + 12, COLOR_NA, FONT_SIZE_NORMAL, ANCHOR_CENTER); heatmap_x += 35; }
      y_offset += SPACING_LARGE; DrawSeparator("sep_heatmap", y_offset, x_offset);
   }
   ...
   if(ShowMagnetProjection)
   {
      ...
      CreateLabel("magnet_level", "Magnet → ---", x_col1, y_offset, COLOR_NEUTRAL_TEXT, FONT_SIZE_NORMAL + 1); CreateLabel("magnet_relation", "(---)", x_col1 + 150, y_offset, COLOR_NA, FONT_SIZE_NORMAL);
      y_offset += SPACING_MEDIUM; DrawSeparator("sep_magnet", y_offset, x_offset);
   }
   ...
   if(ShowMultiTFMagnets)
   {
      ...
      for(int i = 0; i < ArraySize(g_magnet_summary_tfs); i++) { string tf = g_magnet_summary_tf_strings[i]; CreateLabel("mtf_magnet_tf_" + tf, tf + " →", mtf_magnet_x1, y_offset, COLOR_HEADER); CreateLabel("mtf_magnet_relation_" + tf, "---", mtf_magnet_x2, y_offset, COLOR_NA); CreateLabel("mtf_magnet_level_" + tf, "(---)", mtf_magnet_x3, y_offset, COLOR_NA); y_offset += SPACING_MEDIUM; }
   ...
   {
      CreateLabel("hud_activity_header", "HUD ZONE ACTIVITY", x_col1, y_offset, COLOR_HEADER, FONT_SIZE_HEADER); y_offset += SPACING_MEDIUM;
      int hud_activity_x = x_col1 + 20;
      for(int i = 0; i < ArraySize(g_hud_tf_strings); i++) { string tf = g_hud_tf_strings[i]; CreateLabel("hud_activity_tf_" + tf, tf, hud_activity_x, y_offset, COLOR_HEADER, FONT_SIZE_NORMAL, ANCHOR_CENTER); CreateLabel("hud_activity_status_" + tf, "N/A", hud_activity_x, y_offset + 12, COLOR_NA, FONT_SIZE_NORMAL + 2, ANCHOR_CENTER); hud_activity_x += 45; }
      y_offset += SPACING_LARGE; DrawSeparator("sep_hud_activity", y_offset, x_offset);
   }
   ...
   if(ShowConfidenceMatrix)
   {
      ...
      CreateLabel("matrix_hdr_tf", "TF", x_col1, y_offset, COLOR_HEADER); CreateLabel("matrix_hdr_bias", "Bias", x_col1 + 40, y_offset, COLOR_HEADER); CreateLabel("matrix_hdr_zone", "Zone", x_col1 + 100, y_offset, COLOR_HEADER); CreateLabel("matrix_hdr_score", "Score", x_col1 + 160, y_offset, COLOR_HEADER); y_offset += SPACING_MEDIUM;
      for(int i = 0; i < ArraySize(g_matrix_tfs); i++) { string tf = g_matrix_tf_strings[i]; CreateRectangle("matrix_bg_" + tf, x_col1 - 5, y_offset - 2, PANE_WIDTH - 20, 14, clrNONE); CreateLabel("matrix_tf_" + tf, tf, x_col1, y_offset, COLOR_NEUTRAL_TEXT); CreateLabel("matrix_bias_" + tf, "---", x_col1 + 40, y_offset, COLOR_NA); CreateLabel("matrix_zone_" + tf, "---", x_col1 + 100, y_offset, COLOR_NA); CreateLabel("matrix_magnet_" + tf, "---", x_col1 + 160, y_offset, COLOR_NA); y_offset += SPACING_MEDIUM; }
   ...
   if(ShowSessionModule)
   {
      ...
      CreateLabel("session_vol_prefix", "Volatility:", x_col1, y_offset, COLOR_HEADER); CreateRectangle("session_vol_bg", x_col2, y_offset - 2, 60, 14, clrNONE); CreateLabel("session_vol_value", "---", x_col2 + 4, y_offset, COLOR_NEUTRAL_TEXT); y_offset += SPACING_MEDIUM;
   ...
   CreateLabel("trade_status_prefix", "Status:", x_col1, y_offset, COLOR_HEADER); CreateLabel("trade_status_value", "☐ No Trade", x_col2, y_offset, COLOR_NEUTRAL_TEXT); y_offset += SPACING_SEPARATOR;
...
void UpdatePanel()
{
   ...
   if(ShowNewsModule)
   {
      NewsEventData news_items[]; ArrayResize(news_items, MAX_NEWS_ITEMS); int news_count = GetUpcomingNews(news_items);
      for(int i = 0; i < MAX_NEWS_ITEMS; i++) { string idx = IntegerToString(i); if(i < news_count) { UpdateLabel("news_time_" + idx, news_items[i].time, COLOR_TEXT_DIM); UpdateLabel("news_curr_" + idx, news_items[i].currency, COLOR_NEUTRAL_TEXT); ObjectSetString(0, PANE_PREFIX + "news_curr_" + idx, OBJPROP_FONT, "Arial Bold"); UpdateLabel("news_event_" + idx, news_items[i].event_name, COLOR_NEUTRAL_TEXT); UpdateLabel("news_impact_" + idx, NewsImpactToString(news_items[i].impact), NewsImpactToColor(news_items[i].impact)); } else { UpdateLabel("news_time_" + idx, ""); UpdateLabel("news_curr_" + idx, ""); UpdateLabel("news_event_" + idx, ""); UpdateLabel("news_impact_" + idx, ""); } }
   }
   ...
   if(ShowPaneSettings) { string on = "✅", off = "❌"; color on_c = COLOR_BULL, off_c = COLOR_NO_SIGNAL; UpdateLabel("setting_matrix_value", ShowConfidenceMatrix ? on : off, ShowConfidenceMatrix ? on_c : off_c); UpdateLabel("setting_magnet_value", ShowMultiTFMagnets ? on : off, ShowMultiTFMagnets ? on_c : off_c); UpdateLabel("setting_heatmap_value", ShowZoneHeatmap ? on : off, ShowZoneHeatmap ? on_c : off_c); UpdateLabel("setting_hud_activity_value", ShowHUDActivitySection ? on : off, ShowHUDActivitySection ? on_c : off_c); UpdateLabel("setting_reco_value", ShowTradeRecommendation ? on : off, ShowTradeRecommendation ? on_c : off_c); UpdateLabel("setting_emotion_value", ShowEmotionalState ? on : off, ShowEmotionalState ? on_c : off_c); UpdateLabel("setting_alert_value", ShowAlertCenter ? on : off, ShowAlertCenter ? on_c : off_c); }

   // --- Update TF Biases & Zones ---
   if(g_biases_expanded) { string tf_bias_strings[] = {"M15", "M30", "H1", "H2", "H4", "D1"}; ENUM_TIMEFRAMES tf_bias_enums[] = {PERIOD_M15, PERIOD_M30, PERIOD_H1, PERIOD_H2, PERIOD_H4, PERIOD_D1}; double current_price = SymbolInfoDouble(_Symbol, SYMBOL_BID); for(int i = 0; i < ArraySize(tf_bias_strings); i++) { string tf_str = tf_bias_strings[i]; ENUM_TIMEFRAMES tf_enum = tf_bias_enums[i]; double raw_zone_val = iCustom(_Symbol, tf_enum, "AlfredSupDemCore.ex5", 0, 0); double magnet_level = iCustom(_Symbol, tf_enum, "AlfredSupDemCore.ex5", 1, 0); int zone_val = 0; if(raw_zone_val > 0.5) zone_val = 1; else if(raw_zone_val < -0.5) zone_val = -1; UpdateLabel("biases_" + tf_str + "_value", GetBiasLabelFromZone(zone_val), GetBiasColorFromZone(zone_val)); ENUM_ZONE zone_enum = ZONE_NONE; if(zone_val == 1) zone_enum = ZONE_DEMAND; else if(zone_val == -1) zone_enum = ZONE_SUPPLY; UpdateLabel("zone_" + tf_str + "_value", ZoneToString(zone_enum), ZoneToColor(zone_enum)); string magnet_text = GetMagnetRelationLabel(current_price, magnet_level); UpdateLabel("magnet_" + tf_str + "_value", magnet_text, GetMagnetRelationColor(magnet_text)); } }

   // --- Update Zone Interaction Status ---
   ENUM_ZONE_INTERACTION interaction = GetCurrentZoneInteraction(); UpdateLabel("zone_interaction_status", ZoneInteractionToString(interaction), ZoneInteractionToColor(interaction)); ObjectSetInteger(0, PANE_PREFIX + "zone_interaction_highlight", OBJPROP_BGCOLOR, ZoneInteractionToHighlightColor(interaction));
   if(interaction != INTERACTION_NONE) { CachedSupDemData zone_data = GetSupDemData(_Period); UpdateLabel("zone_qual_str_value", StringFormat("%.0f/10", zone_data.strength), COLOR_NEUTRAL_TEXT); string fresh_text = (zone_data.freshness > 0.5) ? "Yes" : "No"; color fresh_color = (zone_data.freshness > 0.5) ? COLOR_BULL : COLOR_BEAR; UpdateLabel("zone_qual_fresh_value", fresh_text, fresh_color); string vol_text = (zone_data.volume > 0.5) ? "Yes" : "No"; color vol_color = (zone_data.volume > 0.5) ? COLOR_BULL : COLOR_BEAR; UpdateLabel("zone_qual_vol_value", vol_text, vol_color); string liq_text = (zone_data.liquidity > 0.5) ? "CONFIRMED" : "No"; color liq_color = (zone_data.liquidity > 0.5) ? clrLime : COLOR_BEAR; UpdateLabel("zone_qual_liq_value", liq_text, liq_color); ObjectSetString(0, PANE_PREFIX + "zone_qual_liq_value", OBJPROP_FONT, (zone_data.liquidity > 0.5) ? "Arial Bold" : "Arial"); }
   else { UpdateLabel("zone_qual_str_value", "N/A", COLOR_NA); UpdateLabel("zone_qual_fresh_value", "N/A", COLOR_NA); UpdateLabel("zone_qual_vol_value", "N/A", COLOR_NA); UpdateLabel("zone_qual_liq_value", "N/A", COLOR_NA); ObjectSetString(0, PANE_PREFIX + "zone_qual_liq_value", OBJPROP_FONT, "Arial"); }

   // --- Update Zone Heatmap ---
   if(ShowZoneHeatmap) { for(int i = 0; i < ArraySize(g_heatmap_tfs); i++) { ENUM_TIMEFRAMES tf = g_heatmap_tfs[i]; string tf_str = g_heatmap_tf_strings[i]; int strength_score = (int)MathRound(iCustom(_Symbol, tf, "AlfredSupDemCore.ex5", 2, 0)); string heatmap_text; if(strength_score >= 8) heatmap_text = "🔥 " + IntegerToString(strength_score); else if(strength_score >= 5) heatmap_text = "🟧 " + IntegerToString(strength_score); else if(strength_score >= 1) heatmap_text = "🟨 " + IntegerToString(strength_score); else heatmap_text = "⚪ " + IntegerToString(strength_score); UpdateLabel("heatmap_status_" + tf_str, heatmap_text, GetHeatColorForStrength(strength_score)); } }

   // --- Update Magnet Projection ---
   if(ShowMagnetProjection) { double magnet_level = GetMagnetProjectionLevel(); double current_price = SymbolInfoDouble(_Symbol, SYMBOL_BID); ENUM_MAGNET_RELATION relation = GetMagnetProjectionRelation(current_price, magnet_level); string level_text = (magnet_level == 0.0) ? "---" : StringFormat("%." + IntegerToString(_Digits) + "f", magnet_level); UpdateLabel("magnet_level", "Magnet → " + level_text, COLOR_NEUTRAL_TEXT); UpdateLabel("magnet_relation", MagnetRelationToString(relation), MagnetRelationToColor(relation)); }

   // --- Update Multi-TF Magnet Summary ---
   if(ShowMultiTFMagnets) { double current_price = SymbolInfoDouble(_Symbol, SYMBOL_BID); double proximity = 2 * _Point; for(int i = 0; i < ArraySize(g_magnet_summary_tfs); i++) { ENUM_TIMEFRAMES tf = g_magnet_summary_tfs[i]; string tf_str = g_magnet_summary_tf_strings[i]; double magnet_level = iCustom(_Symbol, tf, "AlfredSupDemCore.ex5", 1, 0); string relation_text; color relation_color; if(magnet_level > 0.0) { if (current_price > magnet_level + proximity) { relation_text = "Above"; relation_color = clrOrangeRed; } else if (current_price < magnet_level - proximity) { relation_text = "Below"; relation_color = clrLimeGreen; } else { relation_text = "At Magnet"; relation_color = clrGray; } } else { relation_text = "N/A"; relation_color = COLOR_NA; } string level_text = (magnet_level == 0.0) ? "(N/A)" : StringFormat("(%." + IntegerToString(_Digits) + "f)", magnet_level); UpdateLabel("mtf_magnet_relation_" + tf_str, relation_text, relation_color); UpdateLabel("mtf_magnet_level_" + tf_str, level_text, relation_color); } }

   // --- Update HUD Zone Activity ---
   if(ShowHUDActivitySection) { for(int i = 0; i < ArraySize(g_hud_tfs); i++) { ENUM_TIMEFRAMES tf = g_hud_tfs[i]; string tf_str = g_hud_tf_strings[i]; if(tf == PERIOD_D1) { UpdateLabel("hud_activity_status_" + tf_str, "N/A", COLOR_NA); continue; } bool is_active = GetHUDZoneActivity(tf); string status_text = is_active ? "✅" : "❌"; color status_color = is_active ? COLOR_BULL : COLOR_BEAR; UpdateLabel("hud_activity_status_" + tf_str, status_text, status_color); } }

   // --- Update Confidence Matrix ---
   if(ShowConfidenceMatrix) { for(int i = 0; i < ArraySize(g_matrix_tfs); i++) { ENUM_TIMEFRAMES tf = g_matrix_tfs[i]; string tf_str = g_matrix_tf_strings[i]; MatrixRowData data = GetConfidenceMatrixRow(tf); UpdateLabel("matrix_bias_" + tf_str, BiasToString(data.bias), BiasToColor(data.bias)); UpdateLabel("matrix_zone_" + tf_str, ZoneToString(data.zone), ZoneToColor(data.zone)); UpdateLabel("matrix_magnet_" + tf_str, IntegerToString(data.score), COLOR_NEUTRAL_TEXT); ObjectSetInteger(0, PANE_PREFIX + "matrix_bg_" + tf_str, OBJPROP_BGCOLOR, GetConfidenceColor(data.score)); string font_style = (data.score >= 10) ? "Arial Bold" : "Arial"; ObjectSetString(0, PANE_PREFIX + "matrix_tf_" + tf_str, OBJPROP_FONT, font_style); ObjectSetString(0, PANE_PREFIX + "matrix_bias_" + tf_str, OBJPROP_FONT, font_style); ObjectSetString(0, PANE_PREFIX + "matrix_zone_" + tf_str, OBJPROP_FONT, font_style); ObjectSetString(0, PANE_PREFIX + "matrix_magnet_" + tf_str, OBJPROP_FONT, font_style); } }

   // --- Update Trade Recommendation ---
   if(ShowTradeRecommendation) { TradeRecommendation rec = GetTradeRecommendation(); UpdateLabel("reco_action_value", RecoActionToString(rec.action), RecoActionToColor(rec.action)); UpdateLabel("reco_reason_value", rec.reasoning, COLOR_NEUTRAL_TEXT); string reco_obj = PANE_PREFIX + "reco_action_value"; ObjectSetString(0, reco_obj, OBJPROP_FONT, (rec.action == SIGNAL_NONE) ? "Arial Italic" : "Arial Bold"); }

   // --- Update Risk & Positioning ---
   if(ShowRiskModule) { RiskModuleData risk_data = GetRiskModuleData(); UpdateLabel("risk_pct_value", StringFormat("%.1f%%", risk_data.risk_percent), COLOR_NEUTRAL_TEXT); UpdateLabel("risk_pos_size_value", StringFormat("%.2f lots", risk_data.position_size), COLOR_NEUTRAL_TEXT); UpdateLabel("risk_rr_value", risk_data.rr_ratio, COLOR_NEUTRAL_TEXT); ObjectSetString(0, PANE_PREFIX + "risk_rr_value", OBJPROP_FONT, "Arial Bold"); }

   // --- Update Session & Volatility ---
   if(ShowSessionModule) { SessionData s_data = GetSessionData(); UpdateLabel("session_name_value", s_data.session_name, COLOR_SESSION); UpdateLabel("session_overlap_value", s_data.session_overlap, COLOR_NEUTRAL_TEXT); UpdateLabel("session_vol_value", VolatilityToString(s_data.volatility), VolatilityToColor(s_data.volatility)); ObjectSetString(0, PANE_PREFIX + "session_vol_value", OBJPROP_FONT, "Arial Bold"); ObjectSetInteger(0, PANE_PREFIX + "session_vol_bg", OBJPROP_BGCOLOR, VolatilityToHighlightColor(s_data.volatility)); }

   // --- Update Trade Data ---
   LiveTradeData trade_data = FetchTradeLevels(); string price_format = "%." + IntegerToString(_Digits) + "f";
   if(trade_data.trade_exists) { UpdateLabel("trade_entry_value", StringFormat(price_format, trade_data.entry), COLOR_NEUTRAL_TEXT); double sl_pips = (trade_data.sl > 0) ? CalculatePips(trade_data.entry, trade_data.sl) : 0.0; double tp_pips = (trade_data.tp > 0) ? CalculatePips(trade_data.entry, trade_data.tp) : 0.0; string sl_text = (trade_data.sl > 0) ? StringFormat(price_format, trade_data.sl) + StringFormat(" (-%.1f p)", sl_pips) : "---"; string tp_text = (trade_data.tp > 0) ? StringFormat(price_format, trade_data.tp) + StringFormat(" (+%.1f p)", tp_pips) : "---"; UpdateLabel("trade_sl_value", sl_text, COLOR_BEAR); UpdateLabel("trade_tp_value", tp_text, COLOR_BULL); UpdateLabel("trade_status_value", "☑ Active", COLOR_BULL); }
   else { UpdateLabel("trade_entry_value", "---", COLOR_NEUTRAL_TEXT); UpdateLabel("trade_sl_value", "---", COLOR_NEUTRAL_TEXT); UpdateLabel("trade_tp_value", "---", COLOR_NEUTRAL_TEXT); UpdateLabel("trade_status_value", "☐ No Trade", COLOR_NEUTRAL_TEXT); }

   // --- Update Trade Signal ---
   ENUM_TRADE_SIGNAL signal = GetTradeSignal(); string signal_obj = PANE_PREFIX + "trade_signal_status"; ObjectSetString(0, signal_obj, OBJPROP_TEXT, SignalToString(signal)); ObjectSetInteger(0, signal_obj, OBJPROP_COLOR, SignalToColor(signal)); ObjectSetString(0, signal_obj, OBJPROP_FONT, (signal == SIGNAL_NONE) ? "Arial Italic" : "Arial Bold"); ObjectSetInteger(0, signal_obj, OBJPROP_FONTSIZE, (signal == SIGNAL_NONE) ? FONT_SIZE_SIGNAL : FONT_SIZE_SIGNAL_ACTIVE);
   ...
   // --- Update Debug Info ---
   if(ShowDebugInfo) { int active_modules = 0; if(ShowZoneHeatmap) active_modules++; if(ShowMagnetProjection) active_modules++; if(ShowMultiTFMagnets) active_modules++; if(ShowHUDActivitySection) active_modules++; if(ShowConfidenceMatrix) active_modules++; if(ShowTradeRecommendation) active_modules++; if(ShowRiskModule) active_modules++; if(ShowSessionModule) active_modules++; if(ShowNewsModule) active_modules++; if(ShowEmotionalState) active_modules++; if(ShowAlertCenter) active_modules++; if(ShowPaneSettings) active_modules++; string debug_text = StringFormat("Modules Active: %d | %s · %s | Updated: %s", active_modules, _Symbol, EnumToString(_Period), TimeToString(TimeCurrent(), TIME_SECONDS)); UpdateLabel("debug_info", debug_text); }
...
int OnInit()
{
   ...
   for(int i = 0; i < ArraySize(g_timeframes); i++)
   {
      g_compass_cache[i].bias = BIAS_NEUTRAL; g_compass_cache[i].confidence = 0.0;
      g_supdem_cache[i].zone = ZONE_NONE; g_supdem_cache[i].magnet_level = 0.0; g_supdem_cache[i].strength = 0.0; g_supdem_cache[i].freshness = 0.0; g_supdem_cache[i].volume = 0.0; g_supdem_cache[i].liquidity = 0.0;
