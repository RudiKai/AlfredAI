   ...
   for(int i = 0; i < ArraySize(g_timeframes); i++)
   {
      ...
      else
      {
         ...
         if(i == 0 && EnableDebugLogging)
            PrintFormat("PaneCache: HUD -> INVALID_HANDLE"); // Print only once
      }
   }
}
...
MatrixRowData GetConfidenceMatrixRow(ENUM_TIMEFRAMES tf)
{
   ...
   int score = 0;
   if(data.bias == BIAS_BULL && (data.zone == ZONE_DEMAND || data.magnet == RELATION_ABOVE))
      score++;
   if(data.bias == BIAS_BEAR && (data.zone == ZONE_SUPPLY || data.magnet == RELATION_BELOW))
      score++;
   if(data.zone == ZONE_DEMAND && data.magnet == RELATION_ABOVE)
      score++;
   if(data.zone == ZONE_SUPPLY && data.magnet == RELATION_BELOW)
      score++;

   if(score >= 2)
      data.score = CONFIDENCE_STRONG;
   else if(score == 1)
      data.score = CONFIDENCE_MEDIUM;
   else
      data.score = CONFIDENCE_WEAK;
   return data;
}

// MODIFIED: Trade recommendation now includes details on zone quality.
TradeRecommendation GetTradeRecommendation()
{
   ...
   if(strong_bullish_tfs >= 2)
   {
      ...
      {
         MatrixRowData row = GetConfidenceMatrixRow(g_matrix_tfs[i]);
         if(row.score == CONFIDENCE_STRONG && row.bias == BIAS_BULL)
         {
            CachedSupDemData zone_data = GetSupDemData(g_matrix_tfs[i]);
            if(zone_data.zone == ZONE_DEMAND)
            {
               double current_score = zone_data.strength + (zone_data.liquidity > 0.5 ? 5 : 0); // Prioritize liquidity
   ...
   else if(strong_bearish_tfs >= 2)
   {
      ...
      {
         MatrixRowData row = GetConfidenceMatrixRow(g_matrix_tfs[i]);
         if(row.score == CONFIDENCE_STRONG && row.bias == BIAS_BEAR)
         {
            CachedSupDemData zone_data = GetSupDemData(g_matrix_tfs[i]);
...
// MODIFIED: Alert center now detects liquidity grabs as a priority alert.
AlertData GetAlertCenterStatus()
{
   ...
   for(int i = 0; i < ArraySize(g_matrix_tfs); i++)
   {
      ...
      if(row.score == CONFIDENCE_STRONG)
         strong_count++;
      else if(row.score == CONFIDENCE_MEDIUM)
         medium_count++;
   }
...
color MatrixScoreToColor(ENUM_MATRIX_CONFIDENCE s)
{
   switch(s)
   {
      case CONFIDENCE_STRONG:
         return COLOR_MATRIX_STRONG;
      case CONFIDENCE_MEDIUM:
         return COLOR_MATRIX_MEDIUM;
   }
   return COLOR_MATRIX_WEAK;
}
...
void UpdatePanel()
{
   ...
   if(interaction != INTERACTION_NONE)
   {
      ...
      color fresh_color = (zone_data.freshness > 0.5) ? COLOR_BULL : COLOR_BEAR;
      ...
      color liq_color = (zone_data.liquidity > 0.5) ? clrLime : COLOR_BEAR; // Highlight liquidity grab
